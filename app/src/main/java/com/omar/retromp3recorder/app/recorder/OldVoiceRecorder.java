...
public class OldVoiceRecorder {
    ...
    public OldVoiceRecorder(String _filepath, IRecorderCallback _callback) {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        OldVoiceRecorder.this.initBuffer();
                        OldVoiceRecorder.this.initOutput();
                        OldVoiceRecorder.this.initRecorder();
                        ...
                        if (outFile.exists()) {
                            OldVoiceRecorder.this.sendHandlerMessage(STATUS_NORMAL, context.getString(R.string.file_saved_to) + filePath);
                            OldVoiceRecorder.this.sendHandlerMessage(STATUS_NORMAL, String.format(context.getString(R.string.audio_length) +
                                    "= %.1f" + context.getString(R.string.seconds), ((float) counter) / 1000));
                            OldVoiceRecorder.this.sendHandlerMessage(STATUS_NORMAL, String.format(context.getString(R.string.file_size) + " = %.1f " + Constants.KB, ((float) outFile.length()) / 1000));
                            OldVoiceRecorder.this.sendHandlerMessage(STATUS_NORMAL, String.format("%.1f " + Constants.KB + context.getString(R.string.compression_rate), ((float) outFile.length()) / ((float) counter)));

                        } else
                            OldVoiceRecorder.this.sendHandlerMessage(STATUS_ERROR, context.getString(R.string.error_saving_file_to) + filePath);


                    } catch (Exception e) {
                        recording = false;
                        OldVoiceRecorder.this.sendHandlerMessage(STATUS_ERROR, e.getMessage());
                    }
