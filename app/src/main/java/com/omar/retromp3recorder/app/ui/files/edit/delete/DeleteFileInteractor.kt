...
class DeleteFileInteractor @Inject constructor(
    ...
    fun processIO(): ObservableTransformer<DeleteFileView.Input, DeleteFileView.Output> = ObservableTransformer { upstream ->
        upstream.observeOn(scheduler)
                .compose { input ->
                    input
                            .ofType(DeleteFileView.Input.DeleteFile::class.java)
                            .flatMapCompletable { deleteFileUC.execute(it.filePath) }
